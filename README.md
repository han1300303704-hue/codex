...
默认设置为 1024 阵元半波长 ULA、300 GHz、1.5 m 初始距离、20 度初始角度、`v_r=2 m/s`、`v_t=3 m/s`、50 us 跟踪周期、10 dB SNR、2 kHz CFO、100 Hz 相位噪声线宽、3-bit 移相器和 50 次蒙特卡洛试验。运行后在 `results/` 生成：
...
3. 用户物理运动按笛卡尔匀速模型推进，再回写为 `[r, theta, v_r, v_t]`，适合近距离强近场场景。
4. 上一时隙选出的波束会影响下一时隙导频观测的有效 SNR，从而形成真正的闭环跟踪。
5. 强近场大孔径场景启用小范围局部距离-角度扫描精修，避免窄波束下纯 EKF 预测漂移造成失锁。
6. 以 `[r, theta, v_r, v_t, CFO, CPE]` 为状态，使用信息形式 EKF 直接处理复导频的实虚观测。
7. 连续波束或 `B` 位离散相移器波束在预测区域内进行选择；联合方法以不确定性平均增益减去伪焦点守卫区响应为评分。
8. 当增益连续 3 个时隙低于 -3 dB 时，使用宽范围离散码本进行一次测量驱动的重捕获。

固定比较六个场景：理想硬件连续波束、受损未补偿、仅射频补偿、仅量化感知、射频补偿+朴素量化与联合算法。失锁结果同时给出“曾经失锁概率”和“时隙失锁占比”。
